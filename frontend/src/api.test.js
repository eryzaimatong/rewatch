...
// that (a) the old return value on every path is byte-for-byte unchanged —
// no caller anywhere in the app breaks from this pass — and (b) the hidden
// API_META channel actually distinguishes success / HTTP error / network
// error, which is the whole point of doing this.
describe("api.js — three-state result envelope", () => {
  ...
  describe("gettitles (array-returning function)", () => {
    ...
    it("returns [] on a network-layer failure (fetch rejects) without throwing, distinguishable from an HTTP error", async () => {
      ...
      expect(result).toEqual([]);
      expect(getApiMeta(result)).toEqual({ ok: false, type: "network", message: "Failed to fetch" });
    });
  ...
  describe("rateMovie ({ok, data}-returning function)", () => {
    ...
    it("returns {ok: false, data: null} on a network failure rather than an uncaught rejection", async () => {
      ...
      expect(getApiMeta(result)).toEqual({ ok: false, type: "network", message: "offline" });
    });
  });
  ...
  describe("login (always-parse-regardless-of-status function)", () => {
    ...
    it("returns null on a network failure instead of an uncaught rejection — login() had zero guarding before this pass", async () => {
